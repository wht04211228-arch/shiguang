    ...
    <div className="admin-panels">
      <section className="admin-panel">
        ...
        <div className="form-grid two">
          <label><span>订单状态</span><select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="pending">待付款</option><option value="paid">已支付</option><option value="in_progress">制作中</option><option value="fulfilled">已交付</option><option value="cancelled">已取消</option><option value="refunded">已退款</option></select></label>
      ...
      {refund ? (
        <section className="admin-panel refund-admin-panel">
          ...
          <small>“已完成退款”仅记录结果；实际资金操作仍需在对应收款渠道中人工完成并核对。</small>
        </section>
      ) : null}
